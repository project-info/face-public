...
while True:
    ...
    all_distances = all_distances[-300:]
    all_names = all_names[-300:]
    ...
    for past_frame in frames:
        ...
        if(face_names):
            for possible_face in face_names[0]: # dont particularly like this either
                all_distances.append(possible_face[1])
                all_names.append(possible_face[0])
        
        single_face_names = []
        for index in np.argsort(all_distances):
            single_face_names.append((all_names[index], all_distances[index]))
        averaged_face_names = [single_face_names[:5]] # SUS - fix soon does some weird things
